...
2. Before changing a behavior, check the nearest folder `README.md` under `src/`, then the AI
   discovery artifacts in `docs/ai/change-surfaces.json`, `docs/ai/architecture-index.md`, and
   `docs/ai/file-classification-map.json`.
...
6. Reduce guessing by making ownership explicit. If a code or docs change introduces a new
   canonical edit path, update the nearest folder `README.md`, the relevant `docs/ai/` artifact,
   and the shared agent instructions when contributors would otherwise need to infer the rule.
