/**
 * @fileoverview Runs a legacy framework-specific conversation-manager smoke for the reusable
 * Desktop React app workflow: create or reuse `Sample City`, open it in the browser, keep it open
 * through a normal chat turn, then close it and prove the lifecycle truthfully.
 */
...
interface AiSampleCityLifecycleArtifact {
  generatedAt: string;
  command: string;
...
const COMMAND_NAME = "tsx scripts/evidence/aiSampleCityReactLifecycleLiveSmoke.ts";
const ARTIFACT_PATH = path.resolve(
  process.cwd(),
  "runtime/evidence/ai_sample_city_react_lifecycle_live_smoke_report.json"
);
const SESSION_PATH = path.resolve(
  process.cwd(),
  `runtime/ai-sample-city-react-lifecycle-smoke-sessions-${RUN_ID}.json`
);
const STATE_PATH = path.resolve(
  process.cwd(),
  `runtime/tmp-ai-sample-city-react-lifecycle-state-${RUN_ID}.json`
);
const LEDGER_SQLITE_PATH = path.resolve(
  process.cwd(),
  `runtime/tmp-ai-sample-city-react-lifecycle-ledgers-${RUN_ID}.sqlite`
);
const LIVE_RUN_RUNTIME_PATH = path.resolve(
  process.cwd(),
  `runtime/tmp-ai-sample-city-react-lifecycle-live-run-${RUN_ID}`
);
const CONVERSATION_ID = `ai-sample-city-react-lifecycle-smoke-${RUN_ID}`;
const USER_ID = "ai-sample-city-smoke-user";
const USERNAME = "fixtureuser";
const FOLDER_NAME = "Sample City";
...
async function cleanupTrackedSmokeResources(session: ConversationSession | null): Promise<void> {
  ...
  await awaitCleanup(cleanupExecutor.executeWithOutcome({
    ...
    type: "close_browser",
    description: `Close lingering Sample City smoke browser session ${browserSessionId}.`,
    params: {
...
async function main(): Promise<void> {
  ...
  try {
    ...
    assert.ok(await isDirectory(targetFolderPath), "Sample City target folder is missing.");
    assert.ok(
      await pathExists(packageJsonPath),
      "Sample City package.json is missing."
    );
    ...
    assert.ok(
      ...
      `Sample City created duplicate Desktop folders: before=${variantNamesBefore.join(",")} after=${variantNamesAfterOpen.join(",")}`
    );
    ...
    assert.equal(
      ...
      "open",
      "Sample City browser did not remain open during conversation."
    );
    ...
    const trackedBrowserAfterClose =
      ...
      null;
    const artifact: AiSampleCityLifecycleArtifact = {
    ...
    if (artifact.status !== "PASS") {
      throw new Error(
        "Sample City React lifecycle smoke failed. " +
