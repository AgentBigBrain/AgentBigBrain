...
test("ObsidianVaultSink keeps duplicate canonical entities distinct and explains continuity-only notes", async () => {
  ...
  try {
    ...
    assert.match(personNote, /No current profile-memory claims are anchored to this entity right now\./);
...
test("ObsidianVaultSink projects retained profile-memory subjects even when the continuity graph is empty", async () => {
  ...
  try {
    const vaultPath = path.join(tempDir, "vault");
    const runtimeAssetPath = path.join(tempDir, "certificate.pdf");
    await writeFile(runtimeAssetPath, "flare certificate", "utf8");
    const sink = new ObsidianVaultSink({
      vaultPath,
      rootDirectoryName: "AgentBigBrain",
      mirrorAssets: false
    ...
    profileMemory.facts = [
      ...
      },
      {
        ...
        value: "Billy is no longer at Flare Web Design",
        sensitive: false,
        status: "confirmed",
        confidence: 0.95,
        sourceTaskId: "task_billy",
    ...
    await sink.rebuild(buildProjectionSnapshotFixture({
      ...
      mediaArtifacts: [
        {
          artifactId: "media_artifact_certificate",
          provider: "telegram",
          sourceSurface: "telegram_interface",
          kind: "document",
          ...
          fileId: "file_certificate",
          fileUniqueId: "file_certificate_unique",
          mimeType: "application/pdf",
          ...
          width: null,
          height: null,
          checksumSha256: "certificate_sha",
          ownedAssetPath: runtimeAssetPath,
          assetFileName: "media_artifact_certificate.pdf",
          derivedMeaning: {
            summary: "Certificate naming FLARE WEB DESIGN, LLC",
            transcript: null,
            ocrText: "FLARE WEB DESIGN, LLC 801934923",
            entityHints: ["FLARE WEB DESIGN, LLC", "801934923"]
          }
        }
      ],
      entityGraph: {
    ...
    const flareConceptPath = path.join(
      vaultPath,
      "AgentBigBrain",
      "15 Concepts",
      "Flare Web Design.md"
    ...
    const flareConceptNote = await readFile(flareConceptPath, "utf8");
    const mediaArtifactNote = await readFile(
      path.join(
        vaultPath,
        "AgentBigBrain",
        "22 Media Artifacts",
        "2026-04-12 certificate.pdf.md"
      ),
      "utf8"
    );

    assert.match(dashboard, /Profile subjects: 2/);
    assert.match(dashboard, /Derived concepts: 7/);
    ...
    assert.match(billyNote, /Billy is no longer at Flare Web Design/);
    assert.match(billyNote, /\[\[15 Concepts\/Flare Web Design\|Flare Web Design\]\]/);
    ...
    assert.match(flareConceptNote, /Projection lane: derived concepts from retained facts, context observations, and media hints/);
    assert.doesNotMatch(mediaArtifactNote, /- 801934923/);
    assert.match(mediaArtifactNote, /\[\[15 Concepts\/Flare Web Design\|Flare Web Design\]\]/);
    ...
    await assert.rejects(() =>
      access(path.join(vaultPath, "AgentBigBrain", "15 Concepts", "801934923.md"))
    );
    await assert.rejects(() =>
      access(path.join(vaultPath, "AgentBigBrain", "15 Concepts", "Flare WEB Design, LLC.md"))
    );
  } finally {
    await rm(tempDir, { recursive: true, force: true });
  }
