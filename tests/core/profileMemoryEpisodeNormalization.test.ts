...
test("normalizeProfileMemoryEpisodes drops malformed episode payloads and normalizes valid ones", () => {
  const episodes = normalizeProfileMemoryEpisodes([
    {
      ...
      summary: "Billy fell down and the outcome is still unclear.",
      ...
      resolvedAt: null,
      entityRefs: ["entity_billy", "entity_billy"],
      openLoopRefs: ["loop_billy", "loop_billy"],
  ...
  assert.deepEqual(episodes[0]?.entityRefs, ["entity_billy"]);
  assert.deepEqual(episodes[0]?.openLoopRefs, ["loop_billy"]);
