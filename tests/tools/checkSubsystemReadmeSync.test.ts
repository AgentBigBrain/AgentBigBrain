...
test("computeSubsystemReadmeSyncDiagnostics reports missing headings and file references", async () => {
  ...
  assert.deepEqual(diagnostics.issues[0].forbiddenAbsolutePathReferences, []);
  assert.match(diagnostics.issues[0].missingHeadings.join("\n"), /## Inputs/);
});

test("computeSubsystemReadmeSyncDiagnostics reports forbidden absolute local path references", async () => {
  const repoRoot = await mkdtemp(path.join(os.tmpdir(), "agentbigbrain-readme-absolute-path-"));
  await mkdir(path.join(repoRoot, "src/example"), { recursive: true });
  await writeFile(path.join(repoRoot, "src/example/alpha.ts"), "export {};\n", "utf8");
  await writeFile(
    path.join(repoRoot, "src/example/README.md"),
    [
      "## Responsibility",
      "Mentions [alpha.ts](/C:/Users/example/workspace/repo/src/example/alpha.ts).",
      "## Inputs",
      "alpha.ts input",
      "## Outputs",
      "alpha.ts output",
  ...
  assert.equal(diagnostics.issues.length, 1);
  assert.deepEqual(diagnostics.issues[0].missingHeadings, []);
  assert.deepEqual(diagnostics.issues[0].missingFileReferences, []);
  assert.deepEqual(diagnostics.issues[0].forbiddenAbsolutePathReferences, [
    "[alpha.ts](/C:/Users/example/workspace/repo/src/example/alpha.ts)"
  ]);
});
