...
test("renderConversationCommandHelpText includes key command guidance", () => {
  ...
  assert.match(helpText, /\/memory \[list\|resolve\|wrong\|forget\]/);
  assert.match(helpText, /\/review <checkpoint-id>/);
  assert.match(helpText, /\/status \[debug\] - show plain-language job state; add debug for delivery internals/);
  ...
  assert.match(helpText, /\/memory resolve episode_abc123 Riley recovered and is fine now/);
  ...
  assert.match(helpText, /Name your platform shell when you need a specific shell or for non-build shell tasks/i);
  assert.match(helpText, /Browser proof tip: ask me to run the app and verify the homepage UI/i);
  assert.match(helpText, /If Playwright is installed locally, I can use loopback browser verification/i);
  assert.match(helpText, /Status tip: \/status is the normal human view\. Use \/status debug only when you need ack\/final-delivery lifecycle details\./i);
