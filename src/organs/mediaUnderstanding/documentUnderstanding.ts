...
export async function interpretDocumentAttachment(
  ...
  try {
    ...
    const directFieldHints = [fields.presentName, fields.assumedName, fields.identifier]
...
function buildDeterministicDocumentSummary(
  ...
  if (fields.presentName || fields.assumedName || fields.identifier) {
    const parts: string[] = [];
    const documentLabel = normalizedText.toLowerCase().includes("certificate of assumed name")
      ? "Certificate of Assumed Name filing"
      : "document";
    if (fields.presentName) {
      parts.push(`present entity ${fields.presentName}`);
    }
    if (fields.assumedName) {
      parts.push(`assumed name ${fields.assumedName}`);
    }
...
function extractEntitySpecificFields(text: string): {
  ...
  return {
    presentName: extractLabeledField(
      text,
      /The present name of the entity is:\s+(.+?)\s+The identification number assigned by the Bureau is:/i
    ),
    assumedName: extractLabeledField(
      text,
      /Proposed Assumed Name\s+(.+?)\s+Filing Effective Date/i
    ),
    identifier: extractLabeledField(
      text,
      /The identification number assigned by the Bureau is:\s+([0-9]{6,})/i
    )
  };
}
