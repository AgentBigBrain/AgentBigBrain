/**
 * @fileoverview Deterministic Stage 6.85 quality-gate helpers for definition-of-done profiles, verification gates, and truthfulness checks.
 */
...
/**
 * Normalizes proof refs into a stable shape for `stage6_85QualityGatePolicy` logic.
 *
 * **Why it exists:**
 * Centralizes normalization rules for proof refs so call sites stay aligned.
 *
 * **What it talks to:**
...
/**
 * Resolves definition of done profile from available runtime context.
 *
 * **Why it exists:**
 * Prevents divergent selection of definition of done profile by keeping rules in one function.
 *
 * **What it talks to:**
 * - Uses `VerificationCategoryV1` (import `VerificationCategoryV1`) from `./types`.
 *
...
/**
 * Evaluates verification gate and returns a deterministic policy signal.
 *
 * **Why it exists:**
 * Keeps the verification gate policy check explicit and testable before side effects.
 *
 * **What it talks to:**
 * - Uses `VerificationCategoryV1` (import `VerificationCategoryV1`) from `./types`.
 * - Uses `VerificationGateV1` (import `VerificationGateV1`) from `./types`.
 *
...
/**
 * Evaluates truthfulness gate and returns a deterministic policy signal.
 *
 * **Why it exists:**
 * Keeps the truthfulness gate policy check explicit and testable before side effects.
 *
 * **What it talks to:**
 * - Uses local constants/helpers within this module.
 *
