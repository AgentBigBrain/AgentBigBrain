 ...
 */
function parseLoopbackHostnameFromUrl(rawUrl: string | undefined): string | null {
  if (!rawUrl) {
    return null;
  }
  try {
 ...
 */
function isLocalServerStartCommand(command: string | undefined): boolean {
 ...
 * - Uses local param parsing helpers within this module.
 *
 * @param proposal - Proposal under governor review.
 * @returns `true` when the proposal is a bounded managed-process live-run action.
 */
function isManagedProcessLiveRunAction(proposal: GovernanceProposal): boolean {
 ...
 */
function isLoopbackProofAction(proposal: GovernanceProposal): boolean {
  const action = proposal.action;
...
async function getModelAdvisoryRejection(
  ...
  if (isLoopbackProofAction(proposal) || isManagedProcessLiveRunAction(proposal)) {
    return null;
  }
